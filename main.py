...
class Interface:
    ...
    def __init__(self, master):
        ...
        self.mb = Menubutton(self.master, text="Выберите тип броска", bg='green', bd='10', font='12')  # создание кнопки
        # МЕНЮ для выбора типа броска
        self.mb.menu = Menu(self.mb, tearoff=0)
        self.mb["menu"] = self.mb.menu
        self.mb.place(x=20, y=150, anchor='nw', height=50, width=230, bordermode=INSIDE)
        self.mb.menu.add_command(label="Под углом к горизонту", command=self.press)  # создание подпунктов МЕНЮ для
        # выбора типа броска
        ...
        self.mb_vale = Menubutton(self.master, text="Введите данные", bg='green', bd='10', font='12', )  # создание
        # кнопки МЕНЮ для ввода данных
        ...
        self.mb_vale.menu.add_command(label="Ввести с клавиатуры", command=self.press_input)  # создание подпунктов
        # МЕНЮ для выбора способа ввода данных
        ...
        self.visualization = Button(self.master, text="Визуализация данных", bg='green', font='12',
                                    command=self.openGraphica)  # создание кнопки для вызова графического окна с
        # визуализацией данных
        self.visualization.place(x=20, y=330, anchor='nw', height=50, width=230, bordermode=INSIDE)

        self.mb_info = Menubutton(self.master, text="Теория по теме", bg='green', bd='10', font='12', )  # создание
        # кнопки МЕНЮ для выбора способа отображения теории по теме
        ...
        self.mb_info.menu.add_command(label="Информационный ресурс в браузере",
                                      command=lambda aurl=self.url: self.url_open(aurl))  # создание подпунктов МЕНЮ для
        # выбора способа отображения теории по теме
        self.mb_info.menu.add_command(label="Ознакомиться в приложении", command=self.openDialog)
        self.mb_info.menu.add_separator()

        self.mb_save = Menubutton(self.master, text="Сохранить результат", bg='green', bd='10',
                                  font='12', )  # создание
        # кнопки МЕНЮ для выбора способа сохранения результатов
        ...
        self.mb_save.menu.add_command(label="Сохранить в файл", command=self.file_save)  # создание подпунктов МЕНЮ для
        # выбора способа сохранения результатов
        self.mb_save.menu.add_command(
            label="Выслать на электронную почту")  # отсылка на электронную почту не реализована
        self.mb_save.menu.add_separator()

        self.subtitle = Label(self.master, text="Тип броска", font='Arial, 26', bg='blue', fg='chartreuse')  # создание
        # надписей на поле приложения
        ...
        self.b = DoubleVar()
        self.duration = Entry(self.master, font='12',
                              textvariable=self.b)  # создание однострочного текстового поля для
    ...
    def create_canv(self):  # функция создания дочернего окна для графического изображения
        ...
        # создание кнопки для начала анимации траектории полета тела
    ...
    def clickStart(self):  # функция прорисовки (анимации) траектории полета тела
        ...
        if self.subtitle.cget('text') == "Тип броска: Под углом к горизонту":
            ...
            while self.x - 30 <= self.lon * self.scale - 30 or self.y < 630:  # цикл для создания координат точкам и их прорисовка
                ...
                self.i += 0.2
                self.canv.create_text(700, 60, text="Vo = (t * g) / (2 * cos(a))", justify=LEFT,
                                      font='Veranda, 18')  # основные формулы расчетов
    ...
    def read_text(self):  # функция считывания входных данных из файл
...
class Theory:  # класс для ознакомления с теорий
    def __init__(self, master):
        ...
        self.txt = ScrolledText(self.child, width=900, height=700, font='14', wrap=WORD,
                                bg='plum')  # создание текстового поля со скроллом
        self.txt.insert(INSERT, self.insert_text(
            self.txt))  # считывание с файла и запись в текстовое поле теоретического материала
